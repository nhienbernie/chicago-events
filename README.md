...
Chicago Events helps people — especially newcomers — discover what's happening in the city and share their own events. Filter by category, price, and date. View events in a list. Post a yard sale, pickup game, or community meetup in under a minute.
...
\- \*\*Geospatial radius search\*\* using PostGIS `ST\_DWithin` — events filtered by distance from any point in Chicago
...
Filtering by radius must happen in the database — not client-side. `ST\_DWithin` with a spatial index runs in milliseconds regardless of dataset size. Sending the full dataset to the client and filtering there doesn't scale.
...
NEXT\_PUBLIC\_API\_URL=http://localhost:4000
